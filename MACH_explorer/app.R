## MACH EXPLORER ##
# Katharine Sink
# App for exploration and manipulation of MACH dataset

################################################################
### GLOBAL ###
################################################################
# define libraries, global variables and functions
...
### DATA ###
###################
# zenodo url for all data zip files 
zenodo_urls = c(
  "https://zenodo.org/records/15311986/files/attributes.zip", 
  "https://zenodo.org/records/15311986/files/land_cover.zip", 
  "https://zenodo.org/records/15311986/files/MACH_ts.zip", 
  "https://zenodo.org/records/15311986/files/MOPEX_ts.zip",
  "https://zenodo.org/records/15311986/files/shapefile.zip") 

# create temporary working directory
temp_dir = tempdir()

# define subfolders for dataset
subfolders = c("attributes", "land_cover", "MACH_ts", "MOPEX_ts", "shapefile")

# create subfolders in temp directory
lapply(subfolders, function(subfolder) {
  subfolder_path = file.path(temp_dir, subfolder)
...
download_unzip = function(url, dest_folder) {
  temp_file = tempfile(fileext = ".zip")
  download.file(url, temp_file)
  unzip(temp_file, exdir = dest_folder)
  unlink(temp_file) # clean up temporary zip file
}

mapply(download_unzip, zenodo_urls, file.path(temp_dir, subfolders))

# define paths for unzipped files and read files
# annual climate attributes
annual_climate_path = file.path(temp_dir, "attributes", "annual_climate.csv")
annual_climate = read_csv(annual_climate_path)
...
anthropogenic_path = file.path(temp_dir, "attributes", "anthropogenic.csv")
anthropogenic = read_csv(anthropogenic_path)
...
# discharge record for mach files 
discharge_mach_path = file.path(temp_dir, "attributes", "discharge_mach.csv")
discharge_count = read_csv(discharge_mach_path)

# geology attributes
geology_path = file.path(temp_dir, "attributes", "geology.csv")
geology = read_csv(geology_path)
geology = geology[,colSums(is.na(geology)|geology == "") !=nrow(geology)]
geology_names = colnames(geology)[-1]

# hydrology attributes
hydrology_path = file.path(temp_dir, "attributes", "hydrology.csv")
hydrology = read_csv(hydrology_path)
...
monthly_climate_path = file.path(temp_dir, "attributes", "monthly_climate.csv")
monthly_climate = read_csv(monthly_climate_path)
# removes columns that are completely filled with NA or blank
...
overall_climate_path = file.path(temp_dir, "attributes", "overall_climate.csv")
overall_climate = read_csv(overall_climate_path)
# removes columns that are completely filled with NA or blank
...
# regional attributes
regional_path = file.path(temp_dir, "attributes", "regional.csv")
...
site_info_path = file.path(temp_dir, "attributes", "site_info.csv")
site_attributes = read_csv(site_info_path)
site = site_attributes  
site_names = colnames(site)[-(1:2)] 

# soil attributes
soil_path = file.path(temp_dir, "attributes", "soil.csv")
soil = read_csv(soil_path)
soil = soil[,colSums(is.na(soil)|soil == "") !=nrow(soil)]
soil_names = colnames(soil)[-1]

# land cover attributes
lc_dir = file.path(temp_dir, "land_cover")
lc_files = list.files(lc_dir, pattern = "^ANLCD\\d{4}\\.csv", full.names = TRUE)

# polygon shapefiles
shapefile_dir = file.path(temp_dir, "shapefile", "MACH_basins.shp")
...
mach_dir = file.path(temp_dir, "MACH_ts")
# list all files 
...
mopex_dir = file.path(temp_dir, "MOPEX_ts")
...
############################
# spatial data filters for gauges with map (state, lat, long, elev, slope, area)    
 tabPanel(title = "Site Selection",
   ...
   # leaflet map output, gauges that have been filtered will be displayed  
...
# and values shown annual
  ...
  # select water years when radio button for water year is selected
  conditionalPanel(
  ...
  # select calendar years when radio button for calendar year is selected 
  conditionalPanel(
...
# get selected characteristics for selected sites 
    ...
    # attribute selections when single radio button is selected              
    conditionalPanel(
    ...
    # attribute options when monthly radio button is selected 
    conditionalPanel(
    ...
    # attribute options when annual radio button is selected 
      conditionalPanel(
    ...
    # change download button name based on radio button selection 
    conditionalPanel(
      condition = "input.att_data_type == 'single'", 
